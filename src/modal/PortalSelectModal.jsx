...
const PortalSelectModal = ({ show, toggle, schema, formType, formMode, formSubmit, formLabel, option, students }) => {
                                                ...
                                                formType.map((value) => {
                                                    return (
                                                        <div key={value.id}>
                                                            ...
                                                            {
                                                                (value.type === 'select' && value.name === 'name')
                                                                    ? <select {...register(value.name)} id={value.name} onChange={storeInfo} placeholder={value.label} className="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md" >
                                                                        <option value='' selected disabled > Select Voucher </option>
                                                                        {
                                                                            ...
                                                                            option.map((innerArr) => {
                                                                                return (
                                                                                    <>
                                                                                        <option key={innerArr.id} value={innerArr.name} > {innerArr.name} </option>
                                                                                    </>
                                                                                )
                                                                            })
                                                                        }
                                                                    </select>
                                                                    : (value.type == 'select' || value.name === 'id')
                                                                        ? <select {...register(value.name)} id={value.name} onChange={storeInfo} placeholder={value.label} className="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md" >
                                                                            <option value='' selected disabled > Select Student </option>
                                                                            {

                                                                                students.map((innerArr) => {
                                                                                    return (
                                                                                        <>
                                                                                            <option key={innerArr.id} value={innerArr.id} > {innerArr.student_dir.first_name} {innerArr.student_dir.last_name} </option>
                                                                                        </>
                                                                                    )
                                                                                })
                                                                            }
                                                                        </select>
                                                                        : <input key={value.id} type={value.type} {...register(value.name)} id={value.name} onChange={storeInfo} placeholder={value.label} className="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md" />
                                                            }
                                                        ...
                                                        <div className="ml-3">
                                                            ...
                                                            <div className="mt-2 text-sm text-red-700">
                                                                <ul role="list" className="list-disc pl-5 space-y-1">
                                                                    ...
                                                                    {errors.quantity && <li> {errors.quantity?.message} </li>}
