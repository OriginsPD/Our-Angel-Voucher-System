...
const CurrentColumn = () => {
    const { findIssued, issued } = IssuedVoucherApi()
    ...
    useEffect(() => {
        findIssued(authInfo.id)
    }, [])

    console.log(issued)

    return (
        <div className="bg-white h-screen overflow-auto lg:min-w-0 lg:flex-1">
            <div className="pl-4 pr-6 pt-4 pb-4 border-b border-t border-gray-200 sm:pl-6 lg:pl-8 xl:pl-6 xl:pt-6 xl:border-t-0">
                <div className="flex items-center">
                    <h1 className="flex-1 text-lg font-medium">Issued Vouchers</h1>
                                ...
                                <div className="flex items-center justify-between space-x-4">
                                    ...
                                    <div className="min-w-0 space-y-3">
                                        <div className="flex items-center space-x-3">
                                            <span className="h-4 w-4 flex items-center justify-center" aria-hidden="true">
                                                <TicketIcon className="mr-4 flex-shrink-0 h-6 w-6 text-cyan-800" aria-hidden="true" />
                                            </span>
                                            <span className="block">
                                                {items.issued_vouchers.map((innerArr) => {
                                                    return (
                                                        <h2 className="text-sm space-y-4 font-medium">
                                                            <a href="#">
                                                                <span className="absolute inset-0" aria-hidden="true"></span>
                                                                {innerArr.name} <span className="sr-only">Running</span>
                                                            </a>
                                                        </h2>
                                                    )
                                                })}
                                        ...
                                        <a href="#" className="relative group flex items-center space-x-2.5">
                                            ...
                                            <span className="text-sm text-gray-500 group-hover:text-gray-900 font-medium truncate"> Student Owner: {items.student_dir.first_name} {items.student_dir.last_name} </span>
                                        </a>
                                    </div>
                                    ...
                                    <div className="hidden sm:flex flex-col flex-shrink-0 items-end space-y-3">
                                            ...
                                            items.issued_vouchers.map((innerArr) => {
                                                return (
                                                    <p className="flex items-center space-x-4">
                                                        <a href="#" className="relative text-sm text-gray-500 hover:text-gray-900 font-medium"> Price: ${innerArr.price} </a>
                                                        <button type="button" className="relative bg-white rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                                            <span className="sr-only"></span>



                                                            <svg className="text-yellow-300 hover:text-yellow-400 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                        ...
                                        <p className="flex text-gray-500 text-sm space-x-2">
                                            <span>{new Date(items.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                        </p>
                                    </div>
                                </div>
                            </li>
